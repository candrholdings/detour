...
let systemProxy = process.env.http_proxy;
...
function DetourProxy(config, port) {
  ...
  that.liveReload = config.liveReload !== false && require('./util/livereload')().listen();
...
DetourProxy.prototype.loopWatch = function (rule) {
  const
  ...
  sreq = (requestOptions.protocol === 'https:' ? https : http).request(requestOptions, function (sres) {
    ...
    if (sres.statusCode === 404 && options.skipOn404) {
      ...
      if (redirectTtl === 0) {
        ...
        return that.requestToHttp(req, detourUrl, options, redirectTtl - 1, (err, data, status, headers) => {
          if (err) {
            callback(err);
          } else {
            let setCookies = parseSetCookie(sres.headers['set-cookie']);

            // TODO: Should cleverly translate domain/path into original URL, not just strip them out
            setCookies.domain = null;
            ...
            callback(
              ...
              status,
              Object.assign(
                { 'set-cookie': serializeCookie(setCookies) },
                headers
              )
            );
          }
